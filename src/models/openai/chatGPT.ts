...
import { addSystem, getCounter, getHistory, insertInDateBase, getTokens} from "../../helpers/db";
...
export async function chatGPT(chatId :string, question :string, userName :string) :Promise<string> {
  ...
  try {
    return await requestFromAi(history, 'gpt-3.5-turbo-0125', 0.5, 100) ?? 'Прости но я не могу выполнить твой запрос'
  } catch (error) {
